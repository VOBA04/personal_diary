...
Java_com_laba_mydiary_NativeLib_addUser(JNIEnv *env, jobject obj, jstring username,
                                        jstring password) {
...
__attribute__((unused)) JNIEXPORT jboolean JNICALL
Java_com_laba_mydiary_NativeLib_isUserValid(JNIEnv *env, jobject obj, jstring username,
                                            jstring password) {
    const char *username_c = (*env)->GetStringUTFChars(env, username, 0);
    const char *password_c = (*env)->GetStringUTFChars(env, password, 0);
...
__attribute__((unused)) JNIEXPORT jboolean JNICALL
Java_com_laba_mydiary_NativeLib_isUserExists(JNIEnv *env, jobject obj, jstring username) {
    const char *username_c = (*env)->GetStringUTFChars(env, username, 0);
...
Java_com_laba_mydiary_NativeLib_freeUsers(JNIEnv *env, jobject obj) {
    for (int i = 0; i < user_count; i++) {
